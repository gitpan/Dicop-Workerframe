...
A fixed prefix for each password in the keys pace, in hex.
...
Start password, or in case of SET_EXTRACT minimum length to extract.

=item end

End password, or in case of SET_EXTRACT maximum length to extract.
